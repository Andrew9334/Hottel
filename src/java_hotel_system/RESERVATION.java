...
public class RESERVATION {
    // in the reservation table we need add two foreign keys:
    // 1 for the client 
    ...
    // ->alter TABLE reservations ADD CONSTRAINT fk_type_id FOREIGN KEY (type) REFERENCES type(id) on DELETE CASCADE
    ...
    // 2 - when we add a new reservation we need to check if the room is alredy reserved 
    ...
    public boolean addReservation(int client_id, int room_number, String dateIn, String dateOut)
    {
     ...
     public boolean editReservation(int reservationID,int client_id, int room_number, String dateIn, String dateOut)
    {
        PreparedStatement st; 
